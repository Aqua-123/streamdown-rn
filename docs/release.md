...
Publishing is restricted to `main`, serialized by workflow concurrency, and uses npm trusted publishing with GitHub OIDC instead of a long-lived write token. Before the first release from this repository and pipeline, the npm package owner must configure `Aqua-123/streamdown-native` and `.github/workflows/release.yml` as the package's trusted GitHub publisher, and the GitHub `npm` environment must require an explicit reviewer. Keep the repository URL in `package.json` identical to the trusted-publisher repository.
