...
The core installs `react-native-svg` to render Streamdown's exact control icons and task checkmark. Optional examples mention Shiki, RaTeX, beautiful-mermaid, and react-native-webview; streamdown-rn does not install or re-export those renderer engines. Applications must audit the optional versions they choose.
