...
> `git diff --stat 50828ac..HEAD -- src/components/ui src/controls/FullscreenModal.tsx src/controls/TableControls.tsx src/plugins/mermaid/MermaidBlock.tsx tests/parity/ports/streamdown/*fullscreen* tests/parity/ports/streamdown/scroll-lock.test.test.ts`
...
- `src/controls/FullscreenModal.tsx:25-46` uses `SafeAreaView`, a label, close
  button, then `<ScrollView horizontal>` for every child.
- `src/controls/TableControls.tsx:68` places a second horizontal table scroller
  inside it.
- `src/plugins/mermaid/MermaidBlock.tsx:83-92` removes the inline diagram and
...
|---------|---------|---------------------|
| Fullscreen tests | `bun run test --runInBand tests/parity/ports/streamdown/table-fullscreen.native.tsx tests/parity/ports/streamdown/mermaid-fullscreen.test.test.ts tests/parity/ports/streamdown/scroll-lock.test.test.ts` | pass |
| Controls | `bun run test --runInBand src/controls/__tests__/native-controls.test.tsx src/plugins/__tests__/mermaid.test.tsx` | pass |
...
- focused fullscreen tests and fixture scenarios
...
### Step 1: Replace the hard-coded horizontal scroller with two explicit modes

Keep one shared modal shell (safe area, close action, modal semantics). Add the
smallest explicit API that supports:
...
Use document mode. Keep the action strip visible, then a vertical scroller
around the existing horizontal table surface. Clear fullscreen-scoped menu
state on close so reopening starts clean. Keep the plan-003 `Dropdown`
consumer unchanged apart from the required reset.
...
Use canvas mode. Center the diagram, permit the provider surface to receive a
new full-screen layout, and keep pan/zoom controls reachable without clipping.
Do not render a zero-size placeholder during the transition.

**Verify**: a provider component records non-zero full-screen layout on iOS and
Android fixture runs; source fallback remains selectable if rendering failed.

### Step 4: Restore focus after dismissal

Separate close request from focus restoration. Restore exactly once after
native dismissal, with a tested Android fallback where `onDismiss` behavior
differs. Preserve system back and accessibility escape.
