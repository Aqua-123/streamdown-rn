...
describe('native math companion contract', () => {
  ...
  it('has the correct math plugin name and type', () => {
    expect(math).toMatchObject({ name: 'katex', type: 'math' });
  });
  ...
  it('adapts the web rehype plugin contract by omitting the DOM pipeline', () => {
    expect(createMathPlugin().rehypePlugin).toBeUndefined();
  });
