...
describe('Mermaid native fullscreen', () => {
  ...
  it('opens the fullscreen portal when the action is pressed', async () => {
    const { screen } = setup();
  ...
  it('closes fullscreen when the accessible X action is pressed', async () => {
    const { screen } = setup();
    await waitFor(() => expect(screen.getByText('Fullscreen chart')).toBeTruthy());
    fireEvent.press(screen.getByRole('button', { name: 'View fullscreen' }));
  ...
  it('uses native modal isolation instead of mutating browser body scroll', async () => {
    const { screen } = setup();
    await waitFor(() => expect(screen.getByText('Fullscreen chart')).toBeTruthy());
    fireEvent.press(screen.getByRole('button', { name: 'View fullscreen' }));
    expect(screen.UNSAFE_getByType(Modal).props).toMatchObject({ visible: true, transparent: false });
  });

  // parity:219259ff4d671ea9b51987235f3197a03c553777d8050dfa3aaacb5eb8f57d6e
  it('maps Escape semantics to the native modal requestClose event', async () => {
    const { screen } = setup();
    await waitFor(() => expect(screen.getByText('Fullscreen chart')).toBeTruthy());
    fireEvent.press(screen.getByRole('button', { name: 'View fullscreen' }));
  ...
  it('keeps fullscreen open when chart content receives a press', async () => {
    const { screen } = setup();
    await waitFor(() => expect(screen.getByText('Fullscreen chart')).toBeTruthy());
    fireEvent.press(screen.getByRole('button', { name: 'View fullscreen' }));
  ...
  it('keeps fullscreen open when chart content receives a key event', async () => {
    const { screen } = setup();
    await waitFor(() => expect(screen.getByText('Fullscreen chart')).toBeTruthy());
    fireEvent.press(screen.getByRole('button', { name: 'View fullscreen' }));
  ...
  it('prevents the fullscreen action while animation disables controls', async () => {
  ...
  it('renders the native Mermaid chart inside the fullscreen modal', async () => {
    const { screen } = setup();
    await waitFor(() => expect(screen.getByText('Fullscreen chart')).toBeTruthy());
    fireEvent.press(screen.getByRole('button', { name: 'View fullscreen' }));
    const canvas = screen.getByTestId('mermaid-fullscreen-canvas');
    expect(canvas.findByProps({ testID: 'provider-surface' }).findByType(Text).props.children).toBe('Fullscreen chart');
  });
