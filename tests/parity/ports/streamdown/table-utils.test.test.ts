...
describe("Table Utils", () => {
  const formulaPrefixes = [/* parity:fe866cefb9ea389544f4bdabf96c1396fe63c17130d806c04a22e0f696526103 */ ['=', '='], /* parity:d0a9669620036d8b9112f7b6d6bfffe013009bc4b59683f6dd05de964851378d */ ['+', '+'], ['-', '-'], ['@', '@'], ['\t', '\t'], ['\r', '\r'], ['\n', '\n'], ['＝', '＝'], ['＋', '＋'], ['－', '－'], ['＠', '＠']];
  ...
  describe("tableDataToCSV", () => {
    it.each(formulaPrefixes)('neutralizes formula-leading %p cells in headers and rows', (prefix, expectedPrefix) => {
      expect(tableDataToCSV({ headers: [`${prefix}header`], rows: [[`${prefix}row`]] }))
        .toBe(`"'${expectedPrefix}header"\n"'${expectedPrefix}row"`);
    });
  ...
  describe("tableDataToTSV", () => {
    it.each(formulaPrefixes)('neutralizes formula-leading %p cells in headers and rows', (prefix, expectedPrefix) => {
      ...
      expect(tableDataToTSV({ headers: [`${prefix}value`], rows: [[`${prefix}value`]] }))
        .toBe(`'${expectedPrefix === prefix ? escaped : ''}\n'${expectedPrefix === prefix ? escaped : ''}`);
    });
