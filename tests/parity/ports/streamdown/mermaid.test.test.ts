...
describe('case-specific Mermaid native workflow proof', () => {
  // parity:fcb5b3363efc1f1408f749fe06f4250caa21f7ebbaeaac63bb2345eaf7f42c11
  it('initializes the adapter with default config when none is provided', async () => {
    const renderAdapter = jest.fn<MermaidRenderResult, [MermaidRenderRequest]>(() => ({ kind: 'native' as const, content: null }));
    await createMermaidPlugin({ adapter: adapter(renderAdapter) }).render(source);
    expect(renderAdapter.mock.calls[0][0].config).toMatchObject({ theme: 'default', securityLevel: 'strict' });
  });
  // parity:afed512e7adb0fffcbe3918d108f7ecc1248e4b0ac05a2b27759b06a3101e2c2
  it('renders an accessible fullscreen button', async () => {
  ...
  it('closes fullscreen with the close button', async () => {
    const screen = render(block(createMermaidPlugin({ adapter: adapter() })));
    await waitFor(() => expect(screen.getByText('Native diagram')).toBeTruthy());
  ...
  it('closes fullscreen through the outside-diagram native request', async () => {
    const screen = render(block(createMermaidPlugin({ adapter: adapter() })));
    await waitFor(() => expect(screen.getByText('Native diagram')).toBeTruthy());
    fireEvent.press(screen.getByRole('button', { name: 'View fullscreen' }));
  ...
  it('does not close fullscreen when the diagram itself is pressed', async () => {
    const screen = render(block(createMermaidPlugin({ adapter: adapter() })));
    await waitFor(() => expect(screen.getByText('Native diagram')).toBeTruthy());
    fireEvent.press(screen.getByRole('button', { name: 'View fullscreen' }));
  ...
  it('calls onError when Mermaid rendering fails', async () => {
    const onError = jest.fn();
    const screen = render(block(createMermaidPlugin({ onError, adapter: { families: ['flowchart'], render: () => { throw new Error('render failed'); } } })));
  ...
  it('closes fullscreen through the platform Escape-equivalent request', async () => {
    const screen = render(block(createMermaidPlugin({ adapter: adapter() })));
    await waitFor(() => expect(screen.getByText('Native diagram')).toBeTruthy());
    fireEvent.press(screen.getByRole('button', { name: 'View fullscreen' }));
    fireEvent(screen.UNSAFE_getByType(Modal), 'requestClose');
  ...
  it('rejects complex config objects containing functions', () => expect(() => createMermaidPlugin({ config: { callback: () => 'unsafe' } })).toThrow('only JSON values'));
  // parity:5d164e6cc19feb5a1d187c095a93ef563a1784dc214172d649913d4ce5812ef8
  it('keeps multiple plugin configurations isolated', async () => {
    const first = jest.fn<MermaidRenderResult, [MermaidRenderRequest]>(() => ({ kind: 'native' as const, content: null }));
    const second = jest.fn<MermaidRenderResult, [MermaidRenderRequest]>(() => ({ kind: 'native' as const, content: null }));
    await createMermaidPlugin({ config: { theme: 'dark' }, adapter: adapter(first) }).render(source);
    await createMermaidPlugin({ config: { theme: 'forest' }, adapter: adapter(second) }).render(source);
    expect(first.mock.calls[0][0].config.theme).toBe('dark');
    expect(second.mock.calls[0][0].config.theme).toBe('forest');
  });
  it('closes fullscreen through the native system request', async () => {
    const screen = render(block(createMermaidPlugin({ adapter: adapter() }))); await waitFor(() => expect(screen.getByText('Native diagram')).toBeTruthy());
    fireEvent.press(screen.getByRole('button', { name: 'View fullscreen' })); fireEvent(screen.UNSAFE_getByType(Modal), 'requestClose');
    expect(screen.queryByRole('alert', { name: 'Diagram fullscreen' })).toBeNull();
  });
  it('opens a native fullscreen modal from the diagram control', async () => {
    const screen = render(block(createMermaidPlugin({ adapter: adapter() }))); await waitFor(() => expect(screen.getByText('Native diagram')).toBeTruthy());
    fireEvent.press(screen.getByRole('button', { name: 'View fullscreen' })); expect(screen.getByRole('alert', { name: 'Diagram fullscreen' })).toBeTruthy();
  });
  it('keeps fullscreen open when diagram content is pressed', async () => {
    const screen = render(block(createMermaidPlugin({ adapter: adapter() }))); await waitFor(() => expect(screen.getByText('Native diagram')).toBeTruthy());
    fireEvent.press(screen.getByRole('button', { name: 'View fullscreen' })); fireEvent(screen.getByText('Native diagram'), 'press');
    expect(screen.getByRole('alert', { name: 'Diagram fullscreen' })).toBeTruthy();
  });
  it('uses native modal isolation without body scroll mutation', async () => {
    const screen = render(block(createMermaidPlugin({ adapter: adapter() }))); await waitFor(() => expect(screen.getByText('Native diagram')).toBeTruthy());
    fireEvent.press(screen.getByRole('button', { name: 'View fullscreen' })); expect(JSON.stringify(screen.toJSON())).not.toContain('document');
  });
  // parity:b984647fc8e2bf177dbca9233c55343423913d34a9a5030f290fed22b252ee43
  it('downloads Mermaid source through the file capability', async () => {
    const save = jest.fn(() => ({ status: 'success' as const })); const screen = render(block(createMermaidPlugin({ adapter: adapter() }), { capabilities: { files: { save } } }));
    await waitFor(() => expect(screen.getByText('Native diagram')).toBeTruthy()); fireEvent.press(screen.getByRole('button', { name: 'Download diagram' })); fireEvent.press(screen.getByRole('menuitem', { name: 'Download diagram as MMD' }));
    await waitFor(() => expect(save).toHaveBeenCalledWith(expect.objectContaining({ extension: 'mmd' })));
  });
  // parity:1d5ca25b6be182f130915af4ef02c9d2fd60e9923d6b0a50bc72b98edc251f84
  it('downloads adapter PNG bytes through the file capability', async () => {
    const save = jest.fn(() => ({ status: 'success' as const })); const screen = render(block(createMermaidPlugin({ adapter: adapter() }), { capabilities: { files: { save } } }));
    await waitFor(() => expect(screen.getByText('Native diagram')).toBeTruthy()); fireEvent.press(screen.getByRole('button', { name: 'Download diagram' })); fireEvent.press(screen.getByRole('menuitem', { name: 'Download diagram as PNG' }));
    await waitFor(() => expect(save).toHaveBeenCalledWith(expect.objectContaining({ extension: 'png' })));
  });
});
...
describe('Mermaid public native behavior', () => {
  ...
  it('renders without crashing through native theme styling', async () => {
    const screen = render(block(createMermaidPlugin({ adapter: adapter() })));
