...
describe('case-specific security proof', () => {
  // parity:d11b96cd4f1ac6cf2d966e7d078e2ebdcd6262d095bc009d7e041a01a97ccc7d
  it('applies a host URL transform before rendering the native link', () => {
    const result = applySecurityPolicy(parseSemanticDocument('[link](http://example.com)'), {
      urlTransform: (url) => url.replace('http://', 'https://'),
    });
    expect(firstUrl(result)).toBe('https://example.com');
  });
  ...
  it('removes a URL when the transform returns null', () => {
    const result = applySecurityPolicy(parseSemanticDocument('[link](https://example.com)'), { urlTransform: () => null });
    expect(firstUrl(result)).toBeUndefined();
  });

  // parity:5c0f888c8df228b845a5c38b0d3521f30b5918dbd5ecd0c0b7e8a9c794751fb8
  it('renders only elements admitted by allowedElements', () => {
    const result = applySecurityPolicy(parseSemanticDocument('**bold** and *italic*'), { allowedElements: ['p', 'strong'] });
    expect(nodeTypes(result)).toContain('strong');
    expect(nodeTypes(result)).not.toContain('emphasis');
  });

  // parity:8712cedad0f0edc19c0be9d2f706920fd11c2a2364d5b77abeb46bedd5063b6c
  it('passes a safe URL through the default policy unchanged', () => {
    expect(firstUrl(applySecurityPolicy(parseSemanticDocument('[safe](https://example.com)'))))
      .toBe('https://example.com');
  });

  // parity:b4ab76743f91569ad4b2422de2a4fa12910ae5d43a1ee0f2c500f27aeafb7538
  it('keeps only explicitly allowed elements', () => {
    const result = applySecurityPolicy(parseSemanticDocument('**bold** and *italic*'), { allowedElements: ['p', 'strong'] });
    expect(nodeTypes(result)).toContain('strong');
    expect(nodeTypes(result)).not.toContain('emphasis');
  });
  ...
  it('removes an explicitly disallowed element', () => {
    const result = applySecurityPolicy(parseSemanticDocument('**bold** and *italic*'), { disallowedElements: ['strong'] });
    expect(nodeTypes(result)).not.toContain('strong');
    expect(nodeTypes(result)).toContain('emphasis');
  });

  // parity:4d6a1822bfdb5f2e43efe49a0aa075f883cd60035f6ab5236ce028d66f9c67ec
  it('filters elements with the host callback', () => {
    const result = applySecurityPolicy(parseSemanticDocument('# keep\n\n## drop'), {
      allowElement: (node) => node.type !== 'heading' || (node as { type: string; depth: number }).depth === 1,
    });
    expect(result.children.filter((node) => node.type === 'heading').map((node) => node.depth)).toEqual([1]);
  });

  // parity:732dd03faf508792309dbc4a775f62bf5cb2c942e636734cf44912d6ccc65954
  it('unwraps a rejected element while keeping its text', () => {
    const result = applySecurityPolicy(parseSemanticDocument('**visible**'), { disallowedElements: ['strong'], unwrapDisallowed: true });
    expect(nodeTypes(result)).not.toContain('strong');
    expect(JSON.stringify(result)).toContain('visible');
  });

  // parity:579e34f8f1b80fd33a38961b0c0a88d15d0251876e2ea998f403afe8ebc1606e
  it('drops raw HTML when skipHtml is enabled', () => {
    const result = applySecurityPolicy(parseSemanticDocument('before <b>hidden</b> after'), { skipHtml: true });
    expect(nodeTypes(result)).not.toContain('html');
    expect(JSON.stringify(result)).not.toContain('<b>');
  });

  // parity:796e08d59a51630ff15a4e498353835e673fe6cc7f75c02e39625e6d6719b55e
  it('keeps non-disallowed component semantics', () => {
    const result = applySecurityPolicy(parseSemanticDocument('**drop** and *keep*'), { disallowedElements: ['strong'] });
    expect(nodeTypes(result)).toContain('emphasis');
  });

  // parity:024b2f5e15e5d1f9a22498a5a7bfb775e976808e0c125d1e5ebbed02be0717af
  it('composes allowElement with allowedElements', () => {
    const result = applySecurityPolicy(parseSemanticDocument('# keep\n\n## drop\n\ntext'), {
      allowedElements: ['h1', 'h2', 'p'],
      allowElement: (node) => node.type !== 'heading' || (node as { type: string; depth: number }).depth === 1,
    });
    expect(result.children.map((node) => node.type)).toEqual(['heading', 'paragraph']);
  });

  // parity:7436774b5d4f97a27df1071b053d93dd51b190c6d355dd9367d23de3bac501b9
  it('unwraps disallowed content in the component policy', () => {
    const result = applySecurityPolicy(parseSemanticDocument('**visible**'), { disallowedElements: ['strong'], unwrapDisallowed: true });
    expect(JSON.stringify(result)).toContain('visible');
  });

  // parity:32470d0f031aa7e4cc4d96dfaeae9ac9d4e47d6bc727c1d7bedfa954e1def0e0
  it('unwraps content excluded by an allowlist', () => {
    const result = applySecurityPolicy(parseSemanticDocument('**visible**'), { allowedElements: ['p'], unwrapDisallowed: true });
    expect(JSON.stringify(result)).toContain('visible');
  });

  // parity:2af3c1dd881aca14823264f2b88b22296ac0bdbf2ec7142a54e0500956995a14
  it('removes a URL when the host transform returns empty text', () => {
    const result = applySecurityPolicy(parseSemanticDocument('[link](https://example.com)'), { urlTransform: () => '' });
    expect(firstUrl(result)).toBeUndefined();
  });

  // parity:f465da020e812aee6cd07f35289f3d44d254f647ddd9cd1534011d111ecba083
  it('passes a component URL through an identity transform', () => {
    const result = applySecurityPolicy(parseSemanticDocument('[link](https://example.com)'), { urlTransform: (url) => url });
    expect(firstUrl(result)).toBe('https://example.com');
  });

  // parity:071f3188fcf930d8f61f6bb66de04a71f2468dacc15312eb3df589d8b3cfcef8
  it('handles raw HTML with skipHtml at the component boundary', () => {
    const result = applySecurityPolicy(parseSemanticDocument('<i>hidden</i>'), { skipHtml: true });
    expect(JSON.stringify(result)).not.toContain('<i>');
  });

  // parity:5285ea900af18f1772839b4a190e857253a781d91ed3f76be57ed6ad8ef376e4
  it('skips raw HTML nodes at the post-processing boundary', () => {
    const result = applySecurityPolicy(parseSemanticDocument('before <b>hidden</b> after'), { skipHtml: true });
    expect(nodeTypes(result)).not.toContain('html');
    expect(JSON.stringify(result)).not.toContain('<b>');
  });
  ...
  it('keeps raw HTML inert while applying a URL transform', () => {
    const result = applySecurityPolicy(parseSemanticDocument('<b>raw</b> [link](https://example.com)'), { urlTransform: (url) => `${url}?safe=1` });
    expect(firstUrl(result)).toBe('https://example.com?safe=1');
    expect(nodeTypes(result)).not.toContain('html');
  });

  // parity:b0ef11be7fe9e0d4324724b393cc3b6212213fe0eee5ce377e5e0825a3669991
  it('applies URL and allowlist policies together', () => {
    const result = applySecurityPolicy(parseSemanticDocument('**bold** *drop* [link](https://example.com)'), {
      allowedElements: ['p', 'strong', 'a'],
      urlTransform: (url) => `${url}?safe=1`,
    });
    expect(firstUrl(result)).toBe('https://example.com?safe=1');
    expect(nodeTypes(result)).not.toContain('emphasis');
  });
  ...
  it('accepts a local telephone URL', () => {
    expect(sanitizeResourceURL('tel:01392498505', 'link')).toBe('tel:01392498505');
  });

  // parity:aef48a6a20badb9ba70248a7cad408ad787cba34d1371171aa49e26d2829755c
  it('accepts an international telephone URL', () => {
    expect(sanitizeResourceURL('tel:+44-1392-498505', 'link')).toBe('tel:+44-1392-498505');
  });
});
