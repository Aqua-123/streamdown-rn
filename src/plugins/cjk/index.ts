...
const CJK_EMPHASIS = new RegExp(`(${CJK})(\\*\\*|__|\\*|_)([^\\n]+?)\\2(${CJK})`, 'gu');
...
const remarkCjkEmphasis: Plugin<[], Root> = () => (tree) => {
  walk(tree, (node, index, parent) => {
    ...
    for (const match of value.matchAll(CJK_EMPHASIS)) {
      ...
      children.push({ type: 'text', value: match[1] } as Text);
      children.push({
        type: match[2].length === 2 ? 'strong' : 'emphasis',
        children: [{ type: 'text', value: match[3] }],
      } as Parent['children'][number]);
      children.push({ type: 'text', value: match[4] } as Text);
