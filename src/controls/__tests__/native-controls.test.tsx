...
describe('native markdown controls', () => {
  ...
  it('copies code only when a clipboard provider exists', async () => {
    ...
    const screen = render(<Streamdown
      ...
      icons={{ copy: <Text testID="code-copy-icon">copy</Text>, check: <Text testID="code-check-icon">check</Text> }}
    >{markdown}</Streamdown>);
    const copyButton = screen.getByRole('button', { name: 'Copy Code' });
    expect(screen.getByTestId('code-copy-icon')).toBeTruthy();
    expect(screen.queryByTestId('code-check-icon')).toBeNull();
    ...
    await waitFor(() => expect(screen.getByTestId('code-check-icon')).toBeTruthy());
    expect(screen.queryByTestId('code-copy-icon')).toBeNull();
